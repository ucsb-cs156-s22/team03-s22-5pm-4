...
public class ArticleControllerTests extends ControllerTestCase {
        ...
        public void test_that_logged_in_user_can_get_by_id_when_the_id_exists() throws Exception {
                ...
                when(articleRepo.findById(eq(7L))).thenReturn(Optional.of(art));
                ...
                verify(articleRepo, times(1)).findById(eq(7L));
                String expectedJson = mapper.writeValueAsString(art);
        ...
        public void test_that_logged_in_user_can_get_by_id_when_the_id_does_not_exist() throws Exception {
                ...
                when(articleRepo.findById(eq(7L))).thenReturn(Optional.empty());
        ...
        public void logged_in_user_can_get_all_articles() throws Exception {
                ...
                expectedDates.addAll(Arrays.asList(art1, art2));

                when(articleRepo.findAll()).thenReturn(expectedDates);
        ...
        public void an_admin_user_can_post_a_new_article() throws Exception {
                ...
                when(articleRepo.save(eq(art1))).thenReturn(art1);

                // act
                MvcResult response = mockMvc.perform(
                                post("/api/article/post?title=topTenFoods&url=www.google.com&explanation=bestFood&email=nwenstad@ucsb.edu&dateAdded=2022-01-03T00:00:00")
                                                .with(csrf()))
                                .andExpect(status().isOk()).andReturn();

                // assert
                verify(articleRepo, times(1)).save(art1);
                String expectedJson = mapper.writeValueAsString(art1);
                String responseString = response.getResponse().getContentAsString();
                assertEquals(expectedJson, responseString);
        }
